...
{\large Mathématiques — Terminale Bac Pro — ICCER}\\[12pt]
...
\begin{contextebox}
Une entreprise de génie climatique fabrique des raccords en cuivre pour circuits de chauffage. Chaque raccord passe deux contrôles successifs.

\textbf{Contrôle 1 — Étanchéité :} 10\% des raccords présentent un défaut d'étanchéité.

\textbf{Contrôle 2 — Filetage :}
\begin{itemize}[nosep]
  \item Si le raccord a un défaut d'étanchéité, la probabilité d'avoir aussi un défaut de filetage est 0,25.
  \item Si le raccord n'a pas de défaut d'étanchéité, la probabilité d'avoir un défaut de filetage est 0,04.
\end{itemize}

On note : $E$ = ``le raccord a un défaut d'étanchéité'' et $F$ = ``le raccord a un défaut de filetage''.
\end{contextebox}

\vspace{6pt}

\textbf{Q1} \comp{APP} — Traduire les données de l'énoncé en probabilités : donner les valeurs de $P(E)$, $P(\overline{E})$, $P_E(F)$ et $P_{\overline{E}}(F)$. \pts{1 pt}
...
\textbf{Q4} \comp{ANA} — En utilisant la formule des probabilités totales, calculer la probabilité qu'un raccord pris au hasard ait un défaut de filetage. \pts{1 pt}
...
\textbf{Q5} \comp{ANA} — Sur une production de 500 raccords, combien peut-on s'attendre à avoir avec un défaut de filetage ? Le responsable qualité considère que le taux est acceptable si moins de 5\% des raccords sont défectueux en filetage. Est-ce le cas ? \pts{1 pt}
...
\begin{contextebox}
Un installateur thermique fabrique un bac de récupération des condensats à partir d'une plaque carrée de zinc de 30 cm de côté. Il découpe un carré de côté $x$ (en cm) à chaque coin, puis plie les bords vers le haut.
...
\textbf{Q5} \comp{COM} — Conclure : donner la valeur de $x$ qui maximise le volume, le volume maximal, et les dimensions du bac (longueur, largeur, hauteur). L'installateur a besoin d'un bac d'au moins 1\,500 cm$^3$ pour récupérer les condensats. La contrainte est-elle respectée ? \pts{1 pt}
